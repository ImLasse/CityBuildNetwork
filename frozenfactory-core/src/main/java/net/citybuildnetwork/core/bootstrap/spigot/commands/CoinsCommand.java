...
public class CoinsCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if(sender instanceof Player){
            ...
            if(player.hasPermission("frozenfactory.administration.coins")) {
                if(args.length == 0) {
                    cbPlayer.sendMessageToSpigotPlayer(player, Core.getPrefix() + "§7Du hast aktuell §b§o" + CoinsAPI.getCoins(player.getUniqueId().toString()) + " §7coins§8.");
                } else if(args.length==1) {
                    if(args[0] != null && CoinsAPI.existPlayer(UUIDFetcher.getUUID(args[0]).toString())) {
                        cbPlayer.sendMessageToSpigotPlayer(player, Core.getPrefix() + "§7Der Spieler §b§o" + args[0] + "§7 hat aktuell §b§o" + CoinsAPI.getCoins(UUIDFetcher.getUUID(args[0]).toString()) + " §7coins§8.");
                    } else {
                    ...
                    if (args[0].equalsIgnoreCase("add")) {
                        ...
                        cbPlayer.sendMessageToSpigotPlayer(player,Core.getPrefix() + "§7Du hast erfolgreich §b§o" + coins + " §7coins zu §b§o"+ name +" §7hinzugefügt§8.");
                    } else if (args[0].equalsIgnoreCase("remove")) {
                        CoinsAPI.removeCoins(UUIDFetcher.getUUID(name).toString(), coins);
                        cbPlayer.sendMessageToSpigotPlayer(player,Core.getPrefix() + "§7Du hast erfolgreich §b§o" + coins + " §7coins von §b§o"+ name +" §7entfernt§8.");
                    } else if (args[0].equalsIgnoreCase("set")) {
                        CoinsAPI.setCoins(UUIDFetcher.getUUID(name).toString(), coins);
                        cbPlayer.sendMessageToSpigotPlayer(player,Core.getPrefix() + "§7Du hast erfolgreich §b§o" + name + "§8'§bs §7coins auf §b§o"+ coins +" §7geändert§8.");
                    } else {
